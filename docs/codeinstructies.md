...
**Uitnodigingslink (24/09).** Koppelen zonder Firebase-console. De begeleider maakt onder Begeleiding een uitnodiging (`invites/{code}`: coachUid, coachName, clientName, expiresAt na 14 dagen, usedBy). De cliënt opent `/uitnodiging/[code]`, maakt daar zelf een account aan met een eigen wachtwoord (of meldt zich aan), en aanvaardt. In één batch komt dan `coachUids` en `coachNames` op het eigen document, een koppeling `links/{coachUid}_{clientUid}` voor de lijst van de begeleider, en wordt de code verbruikt. De regels staan `get` op een uitnodiging toe zonder account (nooit `list`), verbruiken één keer voor de vervaldatum, en een link enkel door de cliënt met een geldige uitnodiging van precies die begeleider. Links geven zelf geen leesrecht. Handmatig gekoppelde cliënten (`clientUids`) blijven werken. Registreren kan enkel via zo'n link in de app; technisch staat e-mail/wachtwoord-registratie in Firebase Auth open, maar een account zonder koppeling ziet enkel zijn eigen lege gegevens. Op het inlogscherm staat "Wachtwoord vergeten?" (mail in het Nederlands). Getest op 24/09 tegen de live regels, als Thomas: eigen sessies en testcliënt 200, onbestaande code 404, lijst van uitnodigingen 403, link zonder geldige uitnodiging 403, eigen uitnodiging verbruiken 403, uitnodiging leesbaar zonder account. Registreren via de link nog door Thomas te testen.
